...
def build_embedding(
    ...
    bert_weight = (
        bert_embeddings.weight.data
    )

    bert_dim = bert_weight.size(1)

    # ========================================================
    # REDUCED VOCAB EMBEDDING MATRIX
    # ========================================================
    ...
    for new_id in range(vocab_size):
        ...
        original_id = dicts.idxToOrigIdx[new_id]

        new_weight[new_id] = bert_weight[original_id]
    ...
    print(
        f"Reduced embedding matrix size: "
        f"{new_weight.size()}"
    )
